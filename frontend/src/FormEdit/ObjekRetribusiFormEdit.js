...
function FormEditObjekRetribusi({ initialData, onSuccess }) {
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => {
      ...
      if (name === 'panjangTanah' || name === 'lebarTanah') {
        const panjang = name === 'panjangTanah' ? parseFloat(value) : parseFloat(prev.panjangTanah);
        const lebar = name === 'lebarTanah' ? parseFloat(value) : parseFloat(prev.lebarTanah);
      ...
      if (name === 'panjangBangunan' || name === 'lebarBangunan') {
        const panjang = name === 'panjangBangunan' ? parseFloat(value) : parseFloat(prev.panjangBangunan);
        const lebar = name === 'lebarBangunan' ? parseFloat(value) : parseFloat(prev.lebarBangunan);
        if (!isNaN(panjang) && !isNaN(lebar)) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:8000/api/objek-retribusi/${formData.id}`, formData);
